...
model_inventory = YOLO('/home/awiros-docker/alexandria/shelf_monitoring/v1/openvino/fp16/shelfv8_openvino_model/')
...
def person_detection(frame, confidence_threshold, frame_counter):
    ...
    for result in results:
        ...
        print("result id",result.boxes.id)
        ...
        for box, score, label, person_id in zip(boxes, scores, labels, ids):
            ...
            blb.cropped_frame = image[blb.ty:blb.by, blb.tx:blb.bx, :]
            ...
            print("Count Boxes: ",count_boxes)
            if count_boxes == len(boxes):
                blb.attribs["Area 1"] = count1
                blb.attribs["Area 2"] = count2
                blb.attribs["Area 3"] = count3
                blb.attribs["Area 4"] = count4
        ...
        return frame, blobs , [count1, count2, count3, count4]
...
def run(acs_url, broker_url, topic_name):
    ...
    while True:
        ...
        for stream_index,stream in enumerate(meta_obj.streams): 
            print("Stream Image",stream.shape)
            detection_conf = meta_obj.awi_detection_conf[stream_index]
            if stream_index == 0:    
                image, blob , area_count = person_detection(stream , detection_conf , frame_counter)  
                print("image:",image)
                eve = event()
                if (len(blob)>0):
                    for blb in blob:
