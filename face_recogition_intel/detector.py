...
source =r"C:\Users\HP\Downloads\face_recogition_intel2\face_recogition_intel\face_img\preetha3.jpeg"
device ='CPU'
faceDETECT =r"C:\Users\HP\Downloads\face_recogition_intel\face_recogition_intel\model_2022_3\face-detection-retail-0005.xml"
faceLANDMARK= r"C:\Users\HP\Downloads\face_recogition_intel\face_recogition_intel\model_2022_3\landmarks-regression-retail-0009.xml"
faceIDENTIFY= r"C:\Users\HP\Downloads\face_recogition_intel\face_recogition_intel\model_2022_3\face-reidentification-retail-0095.xml"
#faceIDENTIFY FEDENTIFY "model 2022 3/facenet.xml"
...
def draw_face_detection (frame, frame_processor, detections):
    ...
    for roi, landmarks, identity in zip(*detections):
        ...
        print(f"Detected person: {text} Face coordinates: (xmin, ymin, xmax, ymax) = ({xmin}, {ymin}, {xmax}, {ymax})")
...
def image_recognizer (frame, text, identity, face_point, threshold):
    ...
    if identity.id != FaceIdentifier. UNKNOWN_ID:
        if (1-identity.distance) > threshold:
            ...
            credit_text = "Credits points are Added for " + text
            cv2.putText(frame,credit_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1)
        else:
            textsize = cv2.getTextSize("Unknown", cv2. FONT_HERSHEY_SIMPLEX, 0.7, 1)[0]
            cv2.rectangle(frame, (xmin, ymin), (xmin + textsize [0], ymin-textsize [1]), (255, 255, 255), cv2. FILLED)
            cv2.putText(frame, "Un known", (xmin, ymin), cv2. FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1)
...
while True:
    ...
    ret, frame = cap.read()
    if not ret:
        # Handle the case when frames cannot be read (e.g., end of video)
        break
